class Song
  ...
  def self.find_or_create_by_name(find_this_song)
    find_it = self.all.detect {|x| x.name == find_this_song}
    if find_it == nil
      s = self.new 
  ...
  def self.new_from_filename(formatted_file)
    
    c = self.new 
    c.name = formatted_file.split(/[^a-zA-Z\s]|\s-\s/)[1] 
    c.artist_name = formatted_file.split(/[^a-zA-Z\s]|\s-\s/)[0]
    c
  end 
  
  def self.create_from_filename(formatted_file)
